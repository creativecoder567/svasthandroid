    ...
    if (array_key_exists('dateid', $data)){
     ...
     $dtime=date('Y-m-d H:i:s');
      if( strtotime($ctime)<=strtotime('12:00:00') ){
       ...
       $sql="UPDATE  `svasth_enquiry`.`litre` SET  `delivery_status_e` =  '$DELIVERY_STATUS',`active_e` =  '1',`bottle_received_e`='$BRECEIVED',`bottle_count_e`='$BTLE_COUNT',`delivery_time_e`='$dtime' WHERE  `litre`.`id` = '$DATEID' AND `litre`.`userid` = '$USERID' AND `litre`.`orderid` = '$ORDERID'";
      }
